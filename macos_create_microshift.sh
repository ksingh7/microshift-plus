...
podman exec microshift oc -n openshift-ingress set env deployment/router-default --overwrite ROUTER_SUBDOMAIN='${name}-${namespace}.apps.127.0.0.1.nip.io' ROUTER_ALLOW_WILDCARD_ROUTES="true" ROUTER_OVERRIDE_HOSTNAME="true" &> /dev/null
...
podman exec microshift oc scale deployments/packageserver  -n olm --replicas=1 &> /dev/null
